...
export interface ResumenFinanciero {  
  ...
  totalPagar: number;           // baseImponible + iva1
}  
...
// ============================================================================  
/**  
 * Implementa `calcularTotalesPedido`.  
 * Reglas de Negocio:  
...
export function calcularTotalesPedido(pedido: PedidoMovil): ResumenFinanciero {  
  ...
  for (let i = 0; i < pedido.detalles.length; i++) { 
    const detalle = pedido.detalles[i]; 
 
    if (detalle !== undefined) { 
      subtotal = subtotal + (detalle.producto.precioUnitario * detalle.cantidad); 
    } 
  } 
  ...
  return {  
    subtotal: subtotal,  
    descuentoEstudiantil: descuentoEstudiantil,  
    iva15: iva15,  
